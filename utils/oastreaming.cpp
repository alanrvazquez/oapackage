...
inline void print_progress (int csol, int ninput, int nextensions, double Tstart, colindex_t col) {
        ...
        log_print (QUIET, "Extending column %d of array %i/%i ~ %4.1f%% (total collected %d), time %.2f s, %s",
                   col + 1, csol, (int)ninput, 100.0 * ((float)csol) / (float)ninput, (int)nextensions,
                   (get_time_ms () - Tstart), asctime (tminfo));
}
...
int main (int argc, char *argv[]) {
        ...
        log_print (SYSTEM, "Done with column %i, total of %i solutions (time %.2f s))\n", current_col + 1,
                   (int)nextensions, get_time_ms () - Tstart);
